...
public class ResponseExceptionHandler {
	...
	public ResponseEntity<StandardError> genericException(Exception e, HttpServletRequest request) {
		...
		StandardError err = new StandardError(LocalDate.now(), status.value(), "Internal server error", e.getMessage(), request.getRequestURI());
	...
	public ResponseEntity<StandardError> objectNotFound(EntityNotFoundException e, HttpServletRequest request) {
		...
		StandardError err = new StandardError(LocalDate.now(), status.value(), "Object not found", e.getMessage(), request.getRequestURI());
		return ResponseEntity.status(status).body(err);
	}
}
